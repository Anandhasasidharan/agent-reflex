...
**Build time:** 10 weeks · **Difficulty:** Research-adjacent · **GPU required:** No · **LLM:** OpenAI (gpt-4o-mini for classification, any for consistency-sampling)
...
# Set your API key and OTel semconv version
...
```
agent_reflex/
├── common/              # Shared types (StepOTAR, MastMode, MastPlusLabel), config
...
├── recovery/            # 8 playbooks + static selector + contextual bandit (mabwiser)
├── uncertainty/         # Consistency-sampling (N=5, embedding via sentence-transformers)
...
├── tests/               # 83 tests (pytest, pytest-cov, all passing)
...
|-----------|--------|--------|-----------------|
| **Causal graph reconstruction** | OTAR parsing, dependency inference, subtask decomposition | ✅ 5 tests | `pytest tests/test_causal_graph.py -v` |
| **MAST+ Classification** | 18-mode taxonomy, LLM few-shot with 10 examples | ✅ 3 tests | `pytest tests/test_classification.py -v` |
| **Step-level attribution** | Oracle backtracking + CRS on 6-scenario eval set | ✅ 4 tests | `python -m agent_reflex.eval.runner` (requires API key) |
| **Adaptive recovery lift** | Contextual bandit (mabwiser) vs static baseline | ✅ 5 tests | `pytest tests/test_recovery.py -v` |
| **Reliability scoring** | Weighted EWMA with trend & before/after playbook analysis | ✅ 5 tests | `pytest tests/test_reliability.py -v` |
| **Consistency AUROC** | N=5 sampling, embedding agreement, threshold calibration | ✅ 8 tests | `pytest tests/test_uncertainty.py -v` |
| **Topology risk** | 17-mode prediction from fan-out, depth, topology type | ✅ 4 tests | `pytest tests/test_predictive.py -v` |
| **Storage (Postgres)** | 5 SQLAlchemy models + repository with heatmap/breakdown queries | ✅ 8 tests | `pytest tests/test_storage.py -v` |
| **Dashboard API** | FastAPI (9 endpoints): ingest, stats, recovery, reliability, predictive | ✅ 6 tests | `pytest tests/test_dashboard.py -v` |
| **Instrumentation** | OTel GenAI decorators + LangGraph/CrewAI adapters | ✅ 7 tests | `pytest tests/test_instrumentation.py -v` |
| **Types** | MastMode, MastPlusLabel, StepOTAR, CausalGraphNode, AttributionResult | ✅ 6 tests | `pytest tests/test_types.py -v` |
| **Eval scaffolding** | 6 synthetic scenarios with ground-truth root causes | ✅ 5 tests | `pytest tests/test_eval.py -v` |
| **Causal graph viewer** | D3.js interactive graph with root-cause highlighting | ✅ 1 test | `pytest tests/test_causal_viewer.py -v` |
| **Redaction ablation** | Accuracy delta with vs without redaction (7 tests) | ✅ 7 tests | `pytest tests/test_ablation.py -v` |
| **Cross-benchmark** | Who&When + TraceElephant benchmark adapters (9 tests) | ✅ 9 tests | `pytest tests/test_benchmark.py -v` |
| **Full test suite** | 83 tests across all modules | ✅ **83/83 passing (73% cov)** | `pytest --cov=agent_reflex` |

### Attribution Accuracy (Synthetic Eval Set)

Run `OPENAI_API_KEY=sk-... python -m agent_reflex.eval.runner` to produce numbers on this 6-scenario set:

| Scenario | True Mode | Steps | Expected Root Cause |
|----------|-----------|-------|-------------------|
| coord_misaligned_assumptions | Coordination | 3 | step_1 (wrong schema) |
| spec_ambiguous | Specification | 2 | step_1 (ambiguous query) |
| task_hallucination | Task Execution | 1 | step_1 (fabricated fact) |
| infra_rate_limit | Infrastructure | 2 | step_1 (no backoff) |
| verif_overconfident | Verification | 1 | step_1 (false confidence) |
| infra_context_window | Infrastructure | 3 | step_3 (overflow) |

The Who&When benchmark reports that even strong methods historically struggle to exceed high teens in step-level accuracy from logs alone. Newer methods like CHIEF improve on prior baselines without claiming the problem solved. **This is a genuinely hard, research-adjacent problem.** Expecting single-digit or low-double-digit accuracy on the first run is realistic — and worth reporting honestly.
...
OPENAI_API_KEY=sk-... python -m agent_reflex.eval.runner

# Redaction ablation (requires API key)
OPENAI_API_KEY=sk-... python -m agent_reflex.eval.ablation

# Cross-benchmark eval (requires API key + datasets)
OPENAI_API_KEY=sk-... python -m agent_reflex.eval.cross_benchmark
```
...
Output shows mode-level and step-level accuracy for both configurations with a delta. This is a novel empirical result — no published system reports how its own instrumentation choices affect its attribution accuracy.
