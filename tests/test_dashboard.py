...
def test_consistency_score():
    with patch("agent_reflex.uncertainty.consistency.ConsistencyScorer.score") as mock_score:
        ...
        response = client.post("/consistency/score", json={
            "prompt": "What is 2+2?",
            "is_critical": False,
        })
...
def test_recovery_feedback():
    response = client.post("/recovery/feedback", json={
        "session_id": "test_001",
...
def test_reliability_empty():
    response = client.get("/agents/test_agent/reliability")
    assert response.status_code == 200
    data = response.json()
...
def test_predictive_endpoint():
    response = client.get("/predictive/score")
    assert response.status_code == 200
...
def test_get_attribution_placeholder():
    response = client.get("/traces/test_001/attribution")
    assert response.status_code == 200
    data = response.json()
...
def test_get_graph_placeholder():
    response = client.get("/traces/test_001/graph")
    assert response.status_code == 200
    data = response.json()
    assert data["session_id"] == "test_001"
    assert data["graph"] == {}
...
def test_reliability_trend_empty():
    response = client.get("/agents/test_agent/reliability/trend/re_prompt")
    assert response.status_code == 200
    data = response.json()
...
def test_predictive_post():
    response = client.post("/predictive/score", json={
